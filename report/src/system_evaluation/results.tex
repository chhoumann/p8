...
This might be due to the map being unreliable, since when creating the map, the wall separating the room and the common area might cause large fluctuations in the strength of the received signal. 
Consequently, classifications using the map can become unreliable.
We tried to accomodate this by enforcing all map entries to have a standard deviation of $6.5 dCb$.
...
Furthermore, detecting these signal outliers might be difficult if the signal is volatile.
The standard deviations of $6.5 dCb$ was choosen, as we found it to be time-wise suitable when creating the map, yet still yielding promising results.

Furthermore, by having placed beacons only inside of the meeting room, and by having more classification points representing locations inside the meeting room, the classification results might be biased towards \textit{inside} classifications.
One could try to lessen the effect of this bias by placing one or more beacons in the common area or create map of higher resolution.
