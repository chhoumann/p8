...
To execute the first phase of the approach described in Section \ref{sec:first_phase},  we partition the previously described meeting room into a $1m \times 1m$ grid and place beacons as described using painters tape.%Supervisor requests EXACT setup
To collect measurements for the creation of the map, a person holding a Samsung Galaxy A53 5G phone, with the application described in Section \ref{tneisrao}\todo{ref the app} installed and running, traverses systematically through the room and stops at the intersection of each grid-line to collect data. 
...
\end{itemize}
For each of these areas x \todo{how many measurements} data points has been collected and classified on a Samsung Galaxy A53 5G and xxxxxxx.
